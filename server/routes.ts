...
import { z } from "zod";
import { insertClaimSchema, insertPatientInsuranceConnectionSchema, insertInsuranceProviderSchema } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/doctor/treatments", async (req, res) => {
    try {
      ...
      if (!hospitalId) {
        ...
        const patientAdmissions = allAdmissions.filter(a => 
        ...
        if (patientAdmissions.length > 0) {
          ...
          const latestAdmission = patientAdmissions.sort((a, b) => 
  ...
  app.get("/api/patient/insurance/available-providers", async (req, res) => {
    try {
      ...
      const enrichedProviders = await Promise.all(providers.map(async (provider: any) => {
  ...
  app.get("/api/insurance/analytics", async (req, res) => {
    try {
      ...
      const totalPayout = thisMonth.reduce((sum, c) => {
        return sum + (parseFloat(c.amount as any) || 0);
      }, 0);
      ...
      const claimTypeAmounts = {
        emergency: claims.filter(c => c.claimType === "emergency").reduce((sum, c) => sum + (parseFloat(c.amount as any) || 0), 0),
        outpatient: claims.filter(c => c.claimType === "outpatient").reduce((sum, c) => sum + (parseFloat(c.amount as any) || 0), 0),
        inpatient: claims.filter(c => c.claimType === "inpatient").reduce((sum, c) => sum + (parseFloat(c.amount as any) || 0), 0),
        surgery: claims.filter(c => c.claimType === "surgery").reduce((sum, c) => sum + (parseFloat(c.amount as any) || 0), 0),
      };
    ...
    try {
      ...
      for (const connection of connectedPatients) {
        ...
        if (paymentSuccessful) {
          ...
          await storage.updateConnectionBilling(
            connection.id,
            connection.lastBillingDate || new Date(),
            newMissedCount
          );
  ...
  app.get("/api/hospital/all-patients", async (req, res) => {
    try {
      ...
      const enrichedPatients = await Promise.all(patients.map(async (patient) => {
        const treatments = await storage.getTreatmentLogs(patient.id);
        const hospitalTreatments = treatments.filter(t => t.hospitalId === hospital.id);
        const latestTreatment = hospitalTreatments.sort((a, b) => 
          new Date(b.treatmentDate).getTime() - new Date(a.treatmentDate).getTime()
        )[0];
  ...
  app.get("/api/hospital/all-doctors", async (req, res) => {
    try {
      ...
      const enrichedDoctors = await Promise.all(doctors.map(async (doctor) => {
        const allTreatments = await storage.getTreatmentLogs();
        const doctorTreatments = allTreatments.filter(t => 
          t.doctorId === doctor.id && t.hospitalId === hospital.id
        );
        
        const latestTreatment = doctorTreatments.sort((a, b) => 
          new Date(b.treatmentDate).getTime() - new Date(a.treatmentDate).getTime()
        )[0];
